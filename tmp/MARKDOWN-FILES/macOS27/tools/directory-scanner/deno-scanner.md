...
function humanSize(bytes: number): string {
  ...
  return i === 0 ? `${Math.floor(size)} ${units[i]}` : `${size.toFixed(1)} ${units[i]}`;
}
...
async function getEntryInfo(fullPath: string, name: string): Promise<EntryInfo | null> {
  try {
    const st = FOLLOW_SYMLINKS ? await Deno.stat(fullPath) : await Deno.lstat(fullPath);
    return {
      path: fullPath,
      name,
...
function startSpinner(text: string) {
  ...
  spinnerTimer = setInterval(() => {
    ...
    Deno.stdout.writeSync(new TextEncoder().encode(`\r${color(frame, ansi.green)} ${text}`));
  }, 80);
}
...
if (!rootPath) {
  console.error(color(`Error: '${ROOT}' is not a valid directory.`, ansi.bold, ansi.red));
  Deno.exit(1);
}
...
async function buildTreeLines(
  ...
  for (let i = 0; i < entries.length; i++) {
    ...
    if (entry.isDir) {
      const real = await Deno.realPath(entry.path).catch(() => entry.path);
      ...
      lines.push(
        prefix +
          ...
          "  " +
          color(`${ICON.size} ${humanSize(size)}`, ansi.dim, ansi.white),
      );

      const childLines = await buildTreeLines(entry.path, depth + 1, nextPrefix, visited);
      ...
      lines.push(
        prefix +
          ...
          "  " +
          color(`${ICON.size} ${humanSize(entry.size)}`, ansi.dim, ansi.white),
      );
    }
  }
...
console.log(
  color(`  ${ICON.size} mode: `, ansi.dim) +
    color(SCAN_EVERYTHING ? "scan everything" : `max depth ${MAX_DEPTH}`, ansi.green),
);
console.log(color(`  ${"─".repeat(30)}`, ansi.gray));
console.log("");

startSpinner(`${color(ICON.live, ansi.green)} scanning ${color(rootPath, ansi.bold, ansi.white)}`);
...
stopSpinner(color(`${ICON.ok} done`, ansi.green));
...
const summaryTable = makeTable(
  ...
  [
    ...
    ["Depth", SCAN_EVERYTHING ? `${maxDepthReached} (actual)` : `${MAX_DEPTH} (max)`],
...
function run(cmd: string[]): string {
  try {
    const result = new Deno.Command(cmd[0], { args: cmd.slice(1), stdout: "piped" }).outputSync();
...
const systemInfoRows: string[][] = [
  ...
  ["Processor", color(run(["sysctl", "-n", "machdep.cpu.brand_string"]), ansi.white)],
  ["Cores", color(run(["sysctl", "-n", "hw.ncpu"]), ansi.green)],
  ["Memory", color(humanSize(parseInt(run(["sysctl", "-n", "hw.memsize"])) || 0), ansi.green)],
  ["Uptime", color(run(["uptime"]).replace(/^\s*/, ""), ansi.dim, ansi.white)],
...
const rootTable = makeTable(
  ...
  rootEntries.map((e, i) => [
    ...
    e.isDir ? color(`${ICON.dir} dir`, ansi.cyan) : color(`${ICON.file} file`, ansi.white),
    e.isDir ? color(`${e.name}/`, ansi.bold, ansi.cyan) : e.name,
    color(e.isDir ? humanSize(dirSizes.get(e.path) ?? 0) : humanSize(e.size), ansi.green),
  ]),
);
